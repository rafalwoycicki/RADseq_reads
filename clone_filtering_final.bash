...
paste "$directory"Filtered.1."$barcode".table "$directory"Filtered.2."$barcode".table | awk '$1==$3' | cut -f1,2,4 | sed "s/\t/_fc_/" | sed "s/\t/_sc_/" | sed "s/_fc_/\t_fc_\t/" | sed "s/_sc_/\t_sc_\t/" | awk 'length($3)>=130 && length($5)>=140' | awk -F"\t" -vOFS="\t" '{ ($3=substr($3,1,130)) ($5=substr($5,1,140)) };1' | sed "s/\t_sc_\t\([AGCT][AGCT][AGCT][AGCT][AGCT][AGCT][AGCT][AGCT]\)GCTAA/\t\1GC\tTAA/" | sed "s/\t_fc_\t/\t/" > "$directory"Filtered."$barcode".all.table
...
cutadapt -j 10 --length 130 -o "$directory"Short.Filtered."$barcode".p5p7dedupl.1.fq "$directory"Filtered."$barcode".p5p7dedupl.1.fq > FilterShort.1.txt
cutadapt -j 10 --length 130 -e 0 -g P7read5primDBR_MSE1=^NNNNNNNNGC -o "$directory"Short.Filtered."$barcode".p5p7dedupl.2.fq "$directory"Filtered."$barcode".p5p7dedupl.2.fq > FilterShort.2.txt #removing DBR with additional "GC" bases from P7 reads
...
cat "$directory"Rescued.2."$barcode".table | sed "s/\t/_fc_/" | sed "s/_fc_/\t_fc_\t/" | awk 'length($3)>=140' | awk -F"\t" -vOFS="\t" '{ $3=substr($3,1,140) };1' | sed "s/\t_fc_\t\([AGCT][AGCT][AGCT][AGCT][AGCT][AGCT][AGCT][AGCT]\)GCTAA/\t\1GC\tTAA/" > "$directory"Rescued.2."$barcode".all.table
...
cutadapt -j 10 --length 130 -e 0 -g P7read5primDBR_MSE1=^NNNNNNNNGC -o "$directory"Short.Rescued.2."$barcode".p7dedupl.fq "$directory"Rescued.2."$barcode".p7dedupl.fq > ResShort.2.txt
